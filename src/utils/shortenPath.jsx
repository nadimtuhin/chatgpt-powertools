export function shortenPath(path, maxLen = 50) {
  ...
  for (let i = 0; i < folderCount; i++) {
    ...
    const shortenedFolderName =
      folderName.length === 1 ? folderName : folderName.charAt(0);
  ...
  if (result.length > maxLen) {
    ...
    const basename =
      filenameIndex !== -1 ? result.slice(filenameIndex + 1) : result;
    const extensionIndex = basename.lastIndexOf(".");
    const name =
      extensionIndex !== -1 ? basename.slice(0, extensionIndex) : basename;
    const extension =
      extensionIndex !== -1 ? basename.slice(extensionIndex) : "";
